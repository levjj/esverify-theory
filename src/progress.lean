...
lemma exp.progress {R: spec} {P: prop} {Q: propctx} {e: exp} {σ: env}:
  ...
  show is_value (R, σ, e) ∨ ∃s', (R, σ, e) ⟶ s', by begin
    cases e_verified,
    case exp.dvcgen.tru x e' { from
      let s: stack := (R, σ, lett x = true in e') in
      have s ⟶ (R, σ[x↦value.true], e'), from step.tru,
      have ∃s', s ⟶ s', from exists.intro (R, σ[x↦value.true], e') this,
      show is_value s ∨ ∃s', s ⟶ s', from or.inr this
    },
    case exp.dvcgen.fals x e' { from
      let s: stack := (R, σ, letf x = false in e') in
      have s ⟶ (R, σ[x↦value.false], e'), from step.fals,
      have ∃s', s ⟶ s', from exists.intro (R, σ[x↦value.false], e') this,
      show is_value s ∨ ∃s', s ⟶ s', from or.inr this
    },
    case exp.dvcgen.num x n e' { from
      let s: stack := (R, σ, letn x = n in e') in
      have s ⟶ (R, σ[x↦value.num n], e'), from step.num,
      have ∃s', s ⟶ s', from exists.intro (R, σ[x↦value.num n], e') this,
      show is_value s ∨ ∃s', s ⟶ s', from or.inr this
    },
    case exp.dvcgen.func f x R' S' e₁ e₂ { from
      let s: stack := (R, σ, letf f[x] req R' ens S' {e₁} in e₂) in
      have s ⟶ (R, σ[f↦value.func f x R' S' e₁ σ], e₂), from step.closure,
    ...
    case exp.dvcgen.unop op x y e' Q' x_free_in_P _ e'_verified vc_valid { from
      let s: stack := (R, σ, letop y = op[x] in e') in
      ...
      have s ⟶ (R, σ[y↦v'], e'), from step.unop env_has_x op_v_is_v',
    ...
    case exp.dvcgen.binop op x y z e' Q' x_free_in_P y_free_in_P _ e'_verified vc_valid { from
      let s: stack := (R, σ, letop2 z = op[x,y] in e') in
      ...
      let ⟨v', op_v_is_v'⟩ := this in
      have s ⟶ (R, σ[z↦v'], e'), from step.binop env_has_x env_has_y op_v_is_v',
    ...
    case exp.dvcgen.app y f x e' Q' f_free_in_P x_free_in_P _ e'_verified vc_valid { from
      let s: stack := (R, σ, letapp y = f [x] in e') in
      ...
      let s': stack := (gR, gσ[g↦value.func g gx gR gS ge gσ][gx↦vx], ge) · [R, σ, letapp y = f[x] in e'] in
    ...
    case exp.dvcgen.ite x e₂ e₁ Q₁ Q₂ x_free_in_P _ _ vc_valid { from
      let s: stack := (R, σ, exp.ite x e₁ e₂) in
      let ⟨v, env_has_x⟩ := (val_of_free_in_pre_env env_verified fv_R x_free_in_P) in
      ...
      or.elim this (
        ...
        have s ⟶ (R, σ, e₁), from step.ite_true this,
        ...
        have s ⟶ (R, σ, e₂), from step.ite_false this,
        have ∃s', s ⟶ s', from exists.intro (R, σ, e₂) this,
        show is_value s ∨ ∃s', s ⟶ s', from or.inr this
      )
    },
    case exp.dvcgen.return x x_free_in_P { from
      let s: stack := (R, σ, exp.return x) in
...
:=
  ...
  begin
    ...
    case stack.dvcgen.top σ e R P Q env_verified fv_R R_valid e_verified { from
      show is_value (R, σ, e) ∨ ∃s', (R, σ, e) ⟶ s', from exp.progress env_verified fv_R R_valid e_verified
    },
    case stack.dvcgen.cons P P' P'' s' σ σ' f g x y fx R R' S' e₁ e₂ vₓ Q Q' Q''
      ...
      let s := (s' · [R, σ, letapp y = g[x] in e₁]) in
      have s_cons: s = (s' · [R, σ, letapp y = g[x] in e₁]), from rfl,
      or.elim ih
      ( -- step return
        assume s'_is_value: is_value s',
        let ⟨R₂, σ₂, z, v, ⟨s'_return, env2_has_z⟩⟩ := s'_is_value in
        have s_return_cons: s = ((R₂, σ₂, exp.return z) · [R, σ, letapp y = g[x] in e₁]),
      ...
      ( -- step ctx
        ...
        have s ⟶ (s'' · [R, σ, letapp y = g[x] in e₁]), from step.ctx s'_steps,
        have ∃s', s ⟶ s', from exists.intro (s'' · [R, σ, letapp y = g[x] in e₁]) this,
        show is_value s ∨ ∃s', s ⟶ s', from or.inr this
      )
    }
  end
